...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const suggestedOnNewList =
    body?.suggestedOnNewList !== undefined ? Boolean(body.suggestedOnNewList) : undefined;
  ...
  const updated = await prisma.baseItem.update({
    where: { id: params.id },
    data: {
      ...(name !== undefined ? { name } : {}),
      ...
      ...(suggestedOnNewList !== undefined ? { suggestedOnNewList } : {})
    }
  });
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  const updated = await prisma.baseItem.update({ where: { id: params.id }, data: { active: false } });
  return ok(updated);
}
